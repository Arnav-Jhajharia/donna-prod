...
async function _runTurnInner(args: RunTurnArgs): Promise<RunTurnResult> {
  ...
  if (terminatorHit) {
    ...
    // model ended turn without calling a terminator. if the response is *only*
    // text (no tool_use at all), the model is clearly trying to talk to the
    // user — salvage that text as an implicit send_burst rather than showing
    // the cap_hit fallback. this is distinct from leaked-reasoning text
    // adjacent to a tool_use call, which should still be hidden.
    const onlyText =
      lastAssistantContent.length > 0 &&
      lastAssistantContent.every((b) => b.type === "text");
    const salvaged = onlyText
    ...
    if (salvaged.length > 0) {
      console.warn(
        `[brain] salvaged ${salvaged.length} text block(s) as implicit send_burst (model emitted end_turn without terminator)`,
      );
      sends = salvaged;
      terminator = "send_burst";
    } else {
